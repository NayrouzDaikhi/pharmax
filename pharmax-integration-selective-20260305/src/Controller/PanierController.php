    ...
    ): Response
    {
        ...
        if (!$form->isSubmitted() || !$form->isValid()) {
            if ($form->isSubmitted() && !$form->isValid()) {
                ...
                $this->addFlash('error', 'Veuillez corriger les informations de livraison.');
            }
        ...
        try {
            ...
            $rowId = $connection->fetchOne('SELECT id FROM commandes WHERE id = :id', ['id' => (int) $commandeId]);
        ...
        try {
            ...
            $this->addFlash('warning', 'Impossible d\'envoyer l\'email de confirmation pour le moment. La commande a été créée, vous pouvez procéder au paiement.');
        }
        ...
        $logger->info('REDIRECTING TO STRIPE CHECKOUT', [
            'commande_id' => $commandeId,
            'route' => 'app_payment_checkout_start',
        ]);
        return $this->redirectToRoute('app_payment_checkout_start', ['id' => $commandeId]);
    }
}
