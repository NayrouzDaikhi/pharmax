...
class PaymentController extends AbstractController
{
    ...
    /**
     * Direct start of Stripe checkout (GET)
     */
    #[Route('/checkout-start/{id}', name: 'app_payment_checkout_start', methods: ['GET'], requirements: ['id' => '\d+'])]
    public function checkoutStart(int $id, CommandeRepository $commandeRepository): Response
    {
        if (!$this->getUser()) {
            return $this->redirectToRoute('app_login');
        }
        ...
        try {
            ...
            $this->logger->error('Stripe checkout error (start): ' . $e->getMessage());
    ...
    /**
     * Start checkout for the latest pending order of current user
     */
    #[Route('/checkout/latest', name: 'app_payment_checkout_latest', methods: ['GET'])]
    public function checkoutLatest(): Response
    {
        $user = $this->getUser();
        if (!$user) {
            return $this->redirectToRoute('app_login');
        ...
        if (!$commande) {
            $this->addFlash('warning', 'Aucune commande disponible pour le paiement.');
            return $this->redirectToRoute('app_panier_index');
        }

        if (!in_array($commande->getStatut(), ['en_attente', 'en_cours'])) {
            $this->addFlash('warning', 'La dernière commande ne peut pas être payée. Sélectionnez une commande en attente.');
            return $this->redirectToRoute('app_commande_show', ['id' => $commande->getId()]);
        }

        return $this->redirectToRoute('app_payment_checkout_start', ['id' => $commande->getId()]);
    }
    /**
